...
Public Sub RefreshHeatmap()
    ...
    If sVehHdr.Count > tVehCols.Count Then
        MsgBox "Warning: Data Transfer Sheet has " & sVehHdr.Count & _
               " vehicles, but HeatMap Sheet can only accommodate " & tVehCols.Count & " vehicles." & vbCrLf & _
...
Public Function CollectRowLabels(ws As Worksheet, anc As Range) As Collection
    ...
    For r = anc.Row + 2 To lastR
        If Trim$(ws.Cells(r, anc.Column).Value) <> "" Then
            out.Add Trim$(ws.Cells(r, anc.Column).Value)
            emptyRun = 0
        Else
            emptyRun = emptyRun + 1
