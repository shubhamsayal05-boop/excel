...
Public Sub EvaluateAVLStatus()
    ...
    For i = 5 To lastRow
        opCode = wsSheet1.Cells(i, 2).Value
...
Private Function GetTestedAVL(wsHeatmap As Worksheet, opCode As Variant, testedCarName As String) As Double
    ...
    ' Find the column under the "Tested Vehicle" header in row 1 of HeatMap.
    ' The header position is dynamic (depends on how many cars are present).
    avlCol = 0
    lastCol = wsHeatmap.Cells(1, wsHeatmap.Columns.count).End(xlToLeft).Column

    For col = 1 To lastCol
        If InStr(1, Trim(CStr(wsHeatmap.Cells(1, col).Value)), "Tested Vehicle", vbTextCompare) > 0 Then
            avlCol = col
            Exit For
        End If
    Next col

    ' Fallback: search row 2 for the tested car name if header not found
    If avlCol = 0 Then
